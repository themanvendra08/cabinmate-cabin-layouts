...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    const result = await fetch(
      'https://dev-cmapi.zysk.in/api/bookings/cabin-layout',
      {
        method: 'POST',
        headers: {
    ...
    const tempCabins = await fetch(
      `https://dev-cmapi.zysk.in/api/bookings/temp-cabin?franchiseName=${getCabinLayoutPayload.franchiseName}&bookingStartDate=${startDate}&bookingEndDate=${endDate}&cabinType=${cabinType}`,
      {
        headers: {
          'Content-Type': 'application/json',
        },
